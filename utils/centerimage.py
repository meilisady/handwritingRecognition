...
Original file is located at
    https://colab.research.google.com/drive/1radtK4SkUY8sI3f6dPBz-W4ImHFwTnXq
"""
...
def add_padding(img, pad_l, pad_t, pad_r, pad_b):
    ...
    pad_left = np.zeros((height, pad_l), dtype=int)
    img = np.concatenate((pad_left, img), axis=1)

    # Adding padding to the top
    pad_up = np.zeros((pad_t, pad_l + width), dtype=int)
    img = np.concatenate((pad_up, img), axis=0)

    # Adding padding to the right
    pad_right = np.zeros((height + pad_t, pad_r), dtype=int)
    img = np.concatenate((img, pad_right), axis=1)

    # Adding padding to the bottom
    pad_bottom = np.zeros((pad_b, pad_l + width + pad_r), dtype=int)
